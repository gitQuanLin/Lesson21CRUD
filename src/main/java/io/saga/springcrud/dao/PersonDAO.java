package io.saga.springcrud.dao;
...
public class PersonDAO {
    ...
    public Optional<Person> show(String email){
            return jdbcTemplate.query("SELECT * FROM Person WHERE email=?",new Object[]
                    {email},new BeanPropertyRowMapper<>(Person.class)).stream().findAny();
    }
    ...
    public void testMultipleUpdate() {
        ...
        System.out.println("Timer ms: " + (finish - start));
    }
    ...
    public void testBatchUpdate() {
        ...
        long finish = System.currentTimeMillis();
        System.out.println("BatchTimer ms: " + (finish - start));
    }

    private List<Person> create1000people() {
        List<Person> people = new ArrayList<>();
        for (int i = 0; i < 1001; i++) {
            people.add(new Person(i, "Name" + i, 30, "test" + i + "@test.io","Address"));
        }
        return people;
    }
}
